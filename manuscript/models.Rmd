...
There are three ingredients to a model:
`Model = Components + Relationship + Parameters`

The aspects of the world are represented as *components* within the model.
...
Random variables, covariates, predictor, latent variables, features, target, outcome, nodes, ...
Some of these names already carry information about our intent how to model relationships between the components.
The components reflect an aspect of the real world that we are interested in, or that have an important relationship with an aspect that we are interested in.
Some components are merely internal to the model and don't necessarily reflect any real world component.
For example nodes in a neural network, nuisance parameter in statistical models and so on.

Within the model, the components are set mathematically or computationally in *relation* to each other.
For example, the nodes in a neural network are functions of nodes at the lower level of the neural network.
