...
* Then there are so many individual methods that aim to provide causal modeling. One example is "honest causal forests", which are based on random forests and designed to model heterogeneity in treatment effects.[@athey2016recursiv]
* ...
...
* Exclude colliders. In DAG in Figure \@ref(fig:dag-rules), $X_$ is a collider for $Y$ and $X_1$. Adding colliders to a model opens an unwanted path.
