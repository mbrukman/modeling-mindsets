...
Alternative title: Likelihoodism?
Three questions:
...
TODO: Short and precise description of the modeling mindset
...
Statistical inference is based on assuming a distribution for your data, or rather for the variables.
...
First, consider this variable $X$:
```{r}
x = c(2.0, 1.1, 0.3, -0.1, 0.9, 0.9, 1.5,  1.4, 0.1, 0.8, 2.4, -1)
```

Now, we could simply compute the mean of this data:
...
$$\arg \max_{\mu} - \sum_{i=1}^n x^2 - 2x\mu - \mu^2\right)$$
...
Start with assumption of some null hypothesis.
This already involves some distribution and also some specific parameters of this distribution.

Before even looking at data, the frequentist approach says when we would reject the null hypothesis and when not.
Like a cut-off values, in terms of probability.
...
* Or also not in the joint distribution
* Instead we want to know the conditional distribution: For example, the probability for some disease conditional on the treatment that was given.
* That also means we have to frame the problem as conditional likelihood:
* $P(Y | X)$
* Frequentist then makes assumptions about this distribution
* For example, assuming a binomial distribution of Y given X
