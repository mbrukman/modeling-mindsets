...
2012 was one of my first Kaggle competitions.
...
- in supervised ml everything is prediction task
- if not, it does not fit the mindset
- but surprisingly many things work as prediction
- examples
  - predict protein structure from amino acid sequence
  - predict creditworthiness from credit history and demographics
  - classify images, for example images of cells and whether they are cancerous
- very broad: inputs can be images, text, time series
- not as hyped by probably most common: plain old tabular data, think of Excel tables
- output also very varied: predict a numerical outcome (like costs, counts of something, blood values, whatever), classification, labeling (where more than one class can be applied), forecasting as in predicting a time series
- I use term prediction for all these cases
- So the supervised ml approach can actually do a lot
<!-- everything is prediction -->
- what does it do to you if you think like that?
- for myself, I worked a lot in the supervised ML mindset
- so you begin to translate everything as prediction
- TODO: Find some surprising example of translating something as prediction task
...
- sub mindset of [machine learning](#machine-learning)
- The supervised ML mindset is a risk-minimization mindset, where the goals is to minimize some loss function across data.
- focused on prediction, so an external goal
- this means it does not matter what the model looks like, as typical for a machine learning based mindset
...
- the evaluation matches this: it's external and not caring about the internal structure of the model. just the results that it delivers, the accurate predictions.
- it's also task-motivated: the model shall be used for new data, so the evaluation has to happen with data the model has not been trained with.
...
- so culture-wise it's clearly a mindset
- it's also a specific way to see the world: we can only train a model when we know some kind of ground truth
- you can't evaluate anything else
- exaggerating here since we want archetype of mindset
...
- If anything goes, what is considered learning anyways? If not fitting a distribution with random variables?
- Learning means finding the parameters of the assumed distributions -- for statistical models
...
So, a complex hypothesis space with infinite $f$'s.
Great.
...
<!-- evaluation -->
Then last step is the evaluation on test data.
...
This distinguishes supervised machine learning from the other mindsets.
...
Supervised machine learning requries an evaluation with unseen data.
...
- logical conclusion: Automate the search of hyperparams
- makes the search a part of the model, if you will
- 




## Generalization Mindset: What's a good model?
...
The generalization error is the loss evaluated with data not used for training or tuning the model.
This stands in contrast with likelihood-motivated mindsets that encourage to think in distributions and variables.
A note here: Likelihoods also have a central role in machine learning -- but only as a tool not the defining mindset.
...
p3 = p +
  ...
  annotate("label", x = 0, y = -0.8, label = "ML Model", size = 12) +
  scale_y_continuous(limits = c(-1, NA))
```


<!-- supervised vs. stats vs nature -->

- inspired by inspirational paper by Leo Breiman [@breiman2001statistical]
- Also was the seed for this book, many years ago
- paper about what he called  algorithmic vs data modeling
- here with mental models: supervised ML vs statistical modeling
- mental model: nature has some unknown function that maps from features X to Y
...
- in statistical modeling, you try to fill the nature with assumptions, build a model that reflects the inner workings of nature. to some degree

```{r stats, fig.cap = "Statistical Model", fig.height = 2.5, fig.width = 10}
p2
```

- in supervised learning, you are like "nah, I don't rebuild inner workings of nature" instead, it should only reflect on the outside what nature does. mimic nature based on external evaluation.
- so it's a black box
