...
<!-- from world to model -->
Statisticians collect data about the world, and use this data to learn a statistical model.
The statistical model connects the world and the data through random variables:
The world can be simplified by probability distributions;
the data are seen as realizations of the random variables.

<!-- from model to world -->
Statistical modeling is a practical endeavour.
Most of the times, statistical models built out of practical reasons:
To make a decision, to understand some properties of the world a bit better, or to make a prediction.
These goals requires that we interpret the model in place of the world.
But it's not for free.
After all the model building and evaluation, the statistical modeler has to consider two things: representativeness of the model (and the data), the interpretation of probability and causality.

<!-- Part 1: Representativeness -->
Part of thinking about the data-generating process is thinking about representativeness of your data and therefore the model.
Are the data a good sample and representative for the property of the world that you study?
Let's say a statistical modeler analysis data about whether a sepsis screening tool successfully reduced the occurrence of sepsis in a hospital.
They come to the conclusion that the sepsis screening tool helped reduce sepsis-related deaths in this hospital.
Are the data representative for all hospitals in the region, the country, or even the world?
Are the data not even representative for all patients in the hospital, because the tool was only tested with patients in intensive care?
In the statistical modeling mindset, defining the "population" from which your data are a sample of and discussing how representative the data are of this population is crucial for any conclusions drawn from the model.
[Design-based inference](#design-based) fully embraces this mindset of the data being a sample from a larger population.


<!-- Part 2: Interpretation of probability -->
More philosophical is the modelers attitude towards causality, prior probabilities and the likelihood principle.
And also the interpretation of how probability should be interpreted.
...
Statistical modeling is just the foundation, and we have to decide for one of the following statistical sub-mindsets for making the statistical models useful:
...
[Likelihoodism](#likelihoodism) is a lesser known modeling mindset. Like Bayesianism, it adheres to the likelihood principle that says that all the likelihood function captures all evidence from the data (which frequentist inference violates).
However, it comes without prior probabilities.

[Causal inference](#causality) adds causality to statistical modeling. It can be built onto any of the other three mindsets.

A different, yet complimentary, approach is [design-based inference](#design), which focuses on data sampling and experiments, instead of models.

## Strengths

* The statistical modeling mindset is a *language to see the world*. Even if not used for inference, random variables and probability distributions are a useful mental model to perceive the world. 
* Statistical modeling has a long tradition and an immense amount of theoretical foundation, from measurement theory as the basis of probability theory to convergence properties of statistical estimators.
* The data-generating process is an underestimated, yet powerful mental model that encourages mindful modeling and asking the right questions.
...
* Statistical modeling reaches its limits quickly when defining probability distributions becomes difficult. Images and text don't fall neatly into the mindset, and that's where [supervised machine learning](#supervised-ml) shines instead.
* Working in the statistical modeling mindset can be quite "manual" and tedious. Always thinking about the DGP is not easy, and sometimes more automatable mindsets such as supervised machine learning take over.
* Statistical models need a lot of assumptions. Sometimes more, sometimes less. Here are just a few: homoscedasticity, independent and identically distributed data (IID), linearity, independence of errors, lack of (perfect) multicollinearity, ...
* Statistical modeling mindset, when used for prediction, is often outperformed by [supervised machine learning](#supervised-ml). To be fair, that's only true when judging a model originating from the statistical modeling mindset with evaluation approaches from the supervised machine learning mindset. But it also says that high goodness-of-fit and passing all diagnostics is a guarantee for being the best model on all metrics.
*
