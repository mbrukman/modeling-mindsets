...
db2 = datbeta[(datbeta$x > lower) & (datbeta$x < upper),, drop = FALSE]

ggplot(datbeta, aes(x = x)) +
geom_density(fill = "grey", size = 2) +
annotate(x = 0.8, y = 2.3, geom = "label", label = "MAP") +
annotate(x = lower, xend = lower, y = 0, yend = 2.3, geom = "segment") +
annotate(x = upper, xend = upper, y = 0, yend = 2.3, geom = "segment")
...
* Maximum a posteriori estimation: This is the mode (the highest point) of the posteriori distribution. It represents the   
...
* Especially useful when expert knowledge has to be included in the approach.
* Shares all advantages of [statistical models](#statistical-modeling).
* Offers an expressive language to build up models
...
* Allows a holistic probabilistic approach: Through Bayes' theorem, parameters and data can be chained into a probabilistic model. General solver programs can then automatically derive the posterior estimates using MCMC.
* You get some results you would not get with other mindsets: Like certain kinds of variances
...
* Great mindset for updating ones one belief, also outside the statistical inference with computer. A nice model how you should update your own belief.
...
* Always waiting for MCMC chains to finish up.
