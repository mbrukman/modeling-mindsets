...
- For the game Go (but also Chess and some other fully observable games)
- Based on three components
  - Value network: Accepts as input the state, and the output is a single number, the value of a state. value of 1 for certain win, value of -1 for loose. draw is a 0. everything inbetween is about uncertainty who wins.
  - Policy network
  - Monte Carlo Tree Search
- based on self-play
- we start with empty board
- Value network:
  - based on many self-plays we can have a database of many pairs of state + outcome (who won)
  - then it becomes simple supervised training!
  - predict reward from state
  - this then becomes the value of that state
  - has a sense of good board versus bad board
  - the value network is not about acting, just similar to image classification
- Policy network
  - Input is game state
  - Output is the so-called prior policy
  - Basically it's a vector that values each action
  - the higher the output the more weight is on this action being a good move
  - similar output to Monte Carlo tree search
  - Training: in a loop with MCTS
  - MCTS uses the policy network
- Monte Carlo Tree Search
  - Start with root node
  - get for each possible action the policy network output
  - 
...
* Compared to supervised: we train supervised model by telling it what the right action was (right classification, correct value, the ground truth)
  * But in RL, the training data evaluates the actions, which is an implicit way of saying what the right action was. and it does so in its entire chain of actions.
...
## Malicious compliance: Choosing the right reward function
...
- requires that the task of interest can be sufficiently simulated in some way, or that we have a way to run the learning process many times
