...
Even if such data were available, it might be inadequate.
...
### Reinforcement Learning and Everything Else


<!-- ML -->
Reinforcement learning is said to be the third machine learning paradigm.
Next to supervised and unsupervised learning.
And it differs from the other two because of it's unique formulation as a decision-making agent in an interactive environment.

<!-- RL versus supervised -->
Rewards can seem a bit like labels in supervised learning.
Or maybe with a good value function, aren't we in a supervised setup?
We aren't.
Supervised learning still lacks the ability of sequential decision making, balancing exploration and exploitation.
...
At best, this supervised approach could mimick average human players, but it could never explore any new strategies.
There would be no creative freedom.
Relative to reinforcement learning, supervised learning is short-sighted and narrow.
...
In short: it's a fantastic match of mindsets.
For more complex tasks, many reinforcement learning approaches are not adapted.
For example, Go is a too complex game for many algorithms.
Think about all the problems where the state is an images, or a video.
And also the action space might be very large.
Deep learning is perfectly equipped to handle such data.

Deep reinforcement learning, the fusion of the two mindsets, is what got many people excited again about reinforcement learning and AI in general.
Reinforcement learning is made "deep" buy replacing some components with deep neural networks.
This can be a neural network for learning the value function, or learning the policy, for example.
This allows more complex inputs than before.
...
The value network predicts the value (between -1 and +1) of a Go board position.
This value network is trained in a supervised fashion.
From self-play, a data set is created that stores board state + win/loss.
The value network is quite similar to an image classifier, with the Board being the image.

The policy network gives action probabilities based on the Go board.
The agent does not automatically follow the most likely action.
...
<!-- RL in general -->
Deep reinforcement learning is an exciting combination of two mindsets.
