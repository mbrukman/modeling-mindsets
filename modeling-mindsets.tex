...
\tightlist
\item
  The likelihood function is all you need (law of likelihood).
\item
  Statistical models or hypotheses are compared by the ratio of their likelihoods.
\item
...
Frequentist inference also has a subjective component: As we will see later in this chapter, it depends on the intention of analyst, such as stopping criteria for collecting data and how the experiment is set up.
...
Data \(X\) are ``fixed'', and the likelihood is a function of the parameters \(\theta\) \(P(\theta | X) = P(X = x | \theta)\).\footnote{Not interpreted in the Bayesian way. Just that \(X\) are seen as given.}
...
\subsection*{Likelihood Principle}\label{likelihood-principle}}
...
The exponential distribution is a typical contender as distribution assumption, so we would say that the waiting times \(X \sim Exp(\lambda)\), where \(\lambda\) can be interpreted as the inverse wait time, so the expected waiting time would be \(\frac{1}{\lambda}\).
We collected \(n\) waiting times \$x\_1, \ldots, \(x_n\).
...
The likelihood principle is a consequence of 1) the sufficiency principle and 2) the conditionality principle.\textsuperscript{\protect\hyperlink{ref-birnbaum1962foundations}{9}}
...
\subsection*{Law of Likelihood}\label{law-of-likelihood}}


The law of likelihood says\textsuperscript{\protect\hyperlink{ref-hacking1965logic}{10}}:
...
\subsection*{Likelihood Intervals}\label{likelihood-intervals}}
...
\caption{A 1/4 and a 1/16 likelihood ratio interval.}\label{fig:likelihood-interval}
\end{figure}
...
\item
  Toss coin until 3 times tail was observed. The third head comes up at the 12-th toss.
\end{enumerate}
...
I found\textsuperscript{\protect\hyperlink{ref-gandenberger2016not}{11}} a good source on criticism of likelihoodism.
...
\leavevmode\vadjust pre{\hypertarget{ref-birnbaum1962foundations}{}}%
\CSLLeftMargin{9. }
\CSLRightInline{Birnbaum A. On the foundations of statistical inference. Journal of the American Statistical Association. 1962;57(298):269--306. }
