...
Or, as McElreath called them in his book ``Statistical Rethinking'': the ``small'' and the ``large'' world.\textsuperscript{\protect\hyperlink{ref-mcelreath2020statistical}{2}}
...
At least this is what a study claims.\textsuperscript{\protect\hyperlink{ref-kao2001alcohol}{3}}
...
You may have heard about the replication crisis.\textsuperscript{\protect\hyperlink{ref-ioannidis2005most}{4}}
...
This idea of proposing a hypothesis, and then accepting or rejecting it based on a statistical model or test is called null hypothesis significance testing. \footnote{There are two other ``main'' approaches for hypothesis testing: The Fisher approach, and the Neyman-Pearson approach. Null hypothesis significance testing is a combination of the two.\textsuperscript{\protect\hyperlink{ref-perezgonzalez2015fisher}{5}}}
...
\item
  Decide which statistical test to use. This step includes modeling the data. In fact, all statistical tests are statistical models\textsuperscript{\protect\hyperlink{ref-mcelreath2020statistical}{2}}
\item
...
\tightlist
\item
  The likelihood function is all you need.
\item
  Statistical models are compared by the ratio of their likelihoods.
\item
  Data is evidence.
\item
...
The Bayesian wants to hear the sommelier's opinion first before ``analyzing'' the data.
The frequentist asks the sommelier about the tasting procedure: Is the number of wines to taste pre-determined? Does the tasting conclude when the customer found a suitable wine? How are subsequent wines selected?
...
A mindset without the flawed hypothesis testing and p-values?

Welcome to the \textbf{likelihoodist} mindset.

I studied statistics for 5 years, worked as statistician and data scientist for 3 years, then did my PhD studies for 4.5 years.
In those 12 years of statistics I never learned about likelihoodism.
It's fair to say that likelihoodism is the underdog.
Likelihoodism lurks in the shadows while Bayesianism and frequentism are having an epic fight.

Likelihoodism is the purist among the statistical modeling mindsets.
A mindset that completely focuses on the likelihood functions of statistical models.
Likelihoodism is an attempt to make statistics as objective as possible.
It doesn't rely on subjective prior probabilities, like Bayesian inference does.
Frequentist inference also has a subjective component.
The intention of the analyst, like stopping criteria for collecting data, influences the frequentist's results.
But more on that later.

\textbf{All three mindsets use likelihood functions in different ways.}
A short recap: The likelihood function is the same as the data density function, but thes roles of data and parameters within the function are switched.
...
Frequentists use the likelihood to estimate parameters and construct ``imagined'' experiments that teach us about long-run frequencies, such as hypothesis tests and confidence intervals.
Likelihoodists see the likelihood as evidence derived from data for a statistical hypothesis.
Likelihoodists put all the focus on the likelihood and reject the non-likelihood elements from freauentism and Bayesianism:
Likelihoodists reject priors, because they are subjective;
Likelihoodists also reject the frequentists dependence on ``imagined'' experiments, because they violate the likelihood principle and are, arguably, subjective as well.
...
``The likelihood principle asserts that two observations that generate identical likelihood functions are equivalent as evidence''\textsuperscript{\protect\hyperlink{ref-richard2017statistical}{8}}
...
So we might assume that \(X \sim Exp(\lambda)\), where \(\lambda\) can be interpreted as the inverse waiting time.
...
Bayesians get a posterior distribution for \(\lambda\) which is interpreted as belief about the parameter.
...
Both the Bayesian and the likelihoodist adhere to the likelihood principle:
All evidence from the data about \(\lambda\) is contained in the likelihood.
Bayesians use priors, but as long as they don't contain information from the data, it's fine. \footnote{The likelihood principle is of course violated if the data is used to inform the pior. For example empirical priors.}

The frequentist might test whether the parameter is larger than a certain value.
By advising such a test, the frequentist has to ``imagine'' experiments under an assumed distribution under the null hypothesis.
But the null hypothesis is not part of the likelihood.
Frequentists choose the distribution under the null hypothesis based on how they ``imagine'' repetitions of the sampling or the experiment.
That in turn depends on how the experiment was conducted or how the data was collected in the first place.
You will later see a simple coin toss example where the same data from different experiments produce different frequentist conclusions.
...
But the likelihood principle alone, is not enough to build a coherent modeling mindset with.
...
The law of likelihood says\textsuperscript{\protect\hyperlink{ref-hacking1965logic}{9}}:
...
For example, a likelihood ratio of 8 is seen as fairly strong, and 32 or more as strong favoring.\textsuperscript{\protect\hyperlink{ref-richard2017statistical}{8}}
...
\textbf{the law of describes how evidence can be measured.}
...
\caption{1/4 and 1/16 likelihood ratio intervals.}\label{fig:likelihood-interval}
\end{figure}
...
\tightlist
\item
  Toss the coin 12 times. We observe 9 heads out of the 12 tosses.
\item
  Toss the coin until tail is observed 3 times. The third tail appears on the 12-th toss.
\end{enumerate}
...
I found\textsuperscript{\protect\hyperlink{ref-gandenberger2016not}{10}} a good source on criticism of likelihoodism.
