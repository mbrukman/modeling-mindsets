...
At the time I worked at the foundation that had a huge data base of patients with axial spondyloarthritis: doctor visits, blood values, x-ray images, \ldots{}
...
If you could \textbf{force} a random variable to have a certain value, how would the distribution of another random variable change?
A cause is different from a mere association: If you would merely \textbf{observe} a certain value for a random variable, how would the distribution of another random variable look like?\\
We know that a wet lawn does not \emph{cause} rain.
...
But causal inference could also be seen as an ``add-on'' to other mindsets like \protect\hyperlink{frequentism}{frequentist} or \protect\hyperlink{bayesian}{Bayesian} inference, but also for \protect\hyperlink{machine-learning}{machine leanring}.
...
Clearly, the prediction model was not a causal model, and missed, for example, the nonseasonal 2009 flu.\textsuperscript{\protect\hyperlink{ref-lazer2014parable}{13}}
Or otherwise this would mean you can \textbf{cause} flu by hitting the right keys in the Google search bar.
...
Causal inference put's a causal model \emph{on top} of the joint distribution.
...
Variables \(X_2\) and \$X\_3 are direct causes for target \(Y\).
...
DAGs also make an appearance in \protect\hyperlink{bayesian}{Bayesianism}, or more specifically in Bayesian networks aka probabilistic graphical models.
There they \emph{might} represent causality, but they don't have to.
...
One such school is around the works of Judea Pearl, who invented the do-calculus \(P(Y|do(X))\), a mathematical language to express interventions on data.\textsuperscript{\protect\hyperlink{ref-pearl2012calculus}{15}}
...
\item
  Do-calculus, structural causal models, and the backdoor criterion offer a very general modeling language and tools for causal inference.\textsuperscript{\protect\hyperlink{ref-pearl2009causal}{16}}
\item
  The \textbf{potential outcomes framework}\textsuperscript{\protect\hyperlink{ref-athey2016recursive}{17}} is another big causal framework, mostly useful for studying the causal effect of binary variables, and originated from the social sciences. For example drug treatment effects on a disease outcome.
\item
...
\item
  And there are many individual methods for causal inference. To name an individual approach: ``honest causal forests'', for example, are based on random forests which are typically used in in supervised learning. Honest causal forests are designed to model heterogeneity in treatment effects.\textsuperscript{\protect\hyperlink{ref-athey2016recursiv}{\textbf{athey2016recursiv?}}}
\item
\item
  \ldots{}
\end{itemize}
...
\hypertarget{from-causal-to-conditional}{%
\section{From Causal to Conditional}\label{from-causal-to-conditional}}
...
The goal of these trials is to understand the \emph{causal} effect of a treatment on a disease.
So in those trials the random variable ``treatment'' is controlled.
I considered this as a distinct mindset in \protect\hyperlink{design-based}{design-based inference}.
...
Causalists follow these steps\textsuperscript{\protect\hyperlink{ref-pearl2009causal}{16}}:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Formulate causal estimand, like: What is the causal effect of \(X_1\) on \(Y\)?
\item
  Build a causal model: Draw DAG.
\item
  Identify whether causal estimand can be estimated, given the causal model. Requires translating it into conditional probability statements.
\item
...
\caption{To understand the causal effect of X1 on Y, we have to build a regression model with $Y$ as target and X1 and X2 as predictor variables. Roles: Y is the target, X1 the variable of interest, X2 a confounder of X1 and X2, X3 a mediator to the effect of X1 on Y, and X4 is a collider.}\label{fig:dag-rules}
\end{figure}
...
\tightlist
\item
  There is no commonly accepted way to do causal inference yet . Do calculus by Pearl and co is a strong contender though.
\item
  Some people that causality does not exist
\item
